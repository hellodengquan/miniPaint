...
│          new app.Actions.Insert_layer_action(new_layer)  ◄── 主动作         │
│      ])                                                                     │
│  );                                                                         │
└─────────────────────────────────────────────────────────────────────────────┘
    │
...
│  src/js/actions/insert-layer.js:45-170                                      │
└─────────────────────────────────────────────────────────────────────────────┘
    │
    │ 1. 创建 layer 对象，合并默认配置和用户传入的 settings
    │ 2. 检查并删除空白初始图层
    │ 3. 异步加载图片 (new Image())
    │
    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  图片加载完成回调 (insert-layer.js:95-110)                                  │
│                                                                             │
│  layer.link.onload = () => {                                                │
...
│      autoresize_as = [layer.width, layer.height, layer.id, can_automate, true]; ◄── 准备自适应参数
...
| 打开方式 | 入口函数 | Bundle_action 包含的动作 | 说明 |
|----------|----------|--------------------------|------|
| **本地文件** | `open_handler()` | `Insert_layer_action` | 单一动作，但内部会触发 `Autoresize_canvas_action` |
| **URL打开** | `file_open_url_handler()` | `Insert_layer_action` + `Autoresize_canvas_action` | 显式包含两个动作 |
| **Data URL** | `file_open_data_url_handler()` | `Insert_layer_action` + `Autoresize_canvas_action` | 显式包含两个动作 |
| **摄像头** | `open_webcam()` | `Insert_layer_action` + `Autoresize_canvas_action` | 显式包含两个动作 |
| **JSON文件** | `load_json()` | 多个动作组合 | 重置画布 + 插入多个图层 + 选中图层等 |

### 5.3 关键发现：Insert_layer_action 内部的双动作机制

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Insert_layer_action 内部结构                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  async do() {                                                               │
│      // 第一阶段：图层创建                                                   │
│      ┌─────────────────────────────────────────┐                           │
│      │ 1. 创建 layer 对象                      │                           │
│      │ 2. 删除空白初始图层 (Delete_layer_action)│  ◄── 可能的子动作         │
│      │ 3. 异步加载图片                         │                           │
│      │ 4. config.layers.push(layer)            │                           │
│      └─────────────────────────────────────────┘                           │
│                              │                                              │
│                              ▼                                              │
│      // 第二阶段：画布自适应（图片加载完成后）                                │
...
│      │         new Autoresize_canvas_action(...)│  ◄── 第二个动作！         │
...
│  async undo() {                                                             │
│      // 逆序撤销                                                             │
│      │ 1. 撤销 Autoresize_canvas_action (如果存在)                          │
│      │ 2. 移除图层                                                           │
│      │ 3. 恢复删除的空白图层 (如果存在)                                       │
│      └──────────────────────────────────────────────────────────────────    │
│  }                                                                          │
...
│   │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌────────────┐ │  │
│   │  │ open_handler│  │file_open_url│  │ open_webcam │  │  load_json │ │  │
│   │  │   (本地)    │  │   (URL)     │  │  (摄像头)   │  │  (JSON)    │ │  │
│   │  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └─────┬──────┘ │  │
│   │         │                │                │               │        │  │
│   │         └────────────────┴────────────────┘               │        │  │
...
│                              ▼                           ▼                │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │                     actions/insert-layer.js                         │  │
│   │  ┌─────────────────────────────────────────────────────────────┐   │  │
│   │  │ Insert_layer_action.do()                                    │   │  │
│   │  │                                                             │   │  │
│   │  │  1. 创建 layer 对象 ──────────────┐                         │   │  │
│   │  │  2. 删除空白图层 (Delete_layer)   │  ◄── 子动作              │   │  │
│   │  │  3. 加载图片 (异步)               │                         │   │  │
│   │  │  4. 添加图层到 config.layers      │                         │   │  │
│   │  │  5. 自适应画布 (Autoresize_canvas)│  ◄── 关键！内部触发      │   │  │
│   │  │  6. 渲染更新 ◄────────────────────┘                         │   │  │
│   │  └─────────────────────────────────────────────────────────────┘   │  │
│   │                              │                                      │  │
│   │                              ▼                                      │  │
...
│   │                     core/base-layers.js                             │  │
│   │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐     │  │
│   │  │   render()      │  │ convert_layers  │  │ render_object() │     │  │
│   │  │  (主渲染循环)    │  │   _to_canvas()  │  │  (单图层渲染)    │     │  │
│   │  └─────────────────┘  └─────────────────┘  └─────────────────┘     │  │
│   │                              │                                      │  │
│   │                              ▼                                      │  │
│   │  ┌─────────────────────────────────────────────────────────────┐   │  │
│   │  │ config.layers (全局状态数组)                                  │   │  │
│   │  │  - 每个图层包含: id, name, type, link(Image), x, y, ...       │   │  │
│   │  └─────────────────────────────────────────────────────────────┘   │  │
│   │                              │                                      │  │
│   │                              ▼                                      │  │
│   │  ┌─────────────────────────────────────────────────────────────┐   │  │
│   │  │ core/gui/gui-layers.js                                       │   │  │
│   │  │  - render_layers(): 更新图层列表面板                          │   │  │
│   │  └─────────────────────────────────────────────────────────────┘   │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│                                                                            │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
