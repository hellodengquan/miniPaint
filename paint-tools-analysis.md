...
| 参数值 | 实际算法 | 代码注释（错误） | 真实行为 |
|--------|----------|----------------|---------|
| `contiguous = false`（默认） | 栈式 DFS 4邻域 flood fill | `//check only nearest pixels` | ✅ **连通模式**：只填充点击点相邻的相似色区域 |
| `contiguous = true` | O(n) 全图逐像素扫描 | `//global mode - contiguous` | ❌ **注释语义完全反转！**<br>实际是：**非连通全局模式**<br>填充整张图所有相似像素，不管是否连通 |

> 💡 "contiguous" 本意就是"连通的"，但代码里取值正好相反，这是历史遗留的命名 bug
...
| contiguous=true 模式 | **O(n) 全局一次遍历**：不检查连通性，直接比较所有像素（最快） |
| contiguous=false 模式 | 栈式 DFS 只遍历可达区域，平均情况更快 |
| 防抖保护 | fill.js:131 `await new Promise(r => setTimeout(r, 10))` 防止触摸屏连击 |

**隐藏代码缺陷：**
- 第 207-218 行 `for (var j = 0; j < 4; j++)` 内层循环毫无意义
- 效果等价于 `j=0`，j=1,2,3 会错位比较颜色通道
- 但由于相同条件判断会重复命中，没有实际影响

**潜在风险：**
- 超大图（>4000×4000）在 contiguous=true 时，一定是 O(n) 全图遍历
