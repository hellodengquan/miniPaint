...
**入口文件**：[src/js/modules/layer/flatten.js](src/js/modules/layer/flatten.js)
...
```javascript
// 推荐：通过 Action 修改
app.State.do_action(new Actions.Update_layer_action(id, { opacity: 50 }));

// 禁止：直接修改
// config.layer.opacity = 50; // 应抛出警告或错误
```
...
class Base_state_class {
    ...
    notify_change(change) {
        this.change_callbacks.forEach(cb => cb(change));
    }
}
```
...
class Base_state_class {
    create_snapshot() {
        return JSON.stringify({
            layers: config.layers.map(l => ({...l, link: null})),
            width: config.WIDTH,
